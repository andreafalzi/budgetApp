...
function App() {
  ...
  function updateForm(value: ListObjectTypeProps) {
    return setForm((prev) => {
      return { ...prev, ...value };
    });
  }
  ...
  const incomeArray = expenses?.filter((expense: ListObjectTypeProps) => expense.sign === "+");
  const expenseArray = expenses?.filter((expense: ListObjectTypeProps) => expense.sign === "-");

  const queryClient = useQueryClient();

  const { mutate: createExpense } = useMutation(
    (newExpense: ListObjectTypeProps) => createExpenseRequest(newExpense),
    {
      onSettled: () => {
